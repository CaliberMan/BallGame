...
void redraw_item(GtkDrawingArea *area, GdkRectangle *old, GdkRectangle *new)
{
    ...
    // Redraws the disc.
    gtk_widget_queue_draw_area((GtkWidget*)area,
...
gboolean on_move_disc(gpointer user_data)
{
    ...
    else
    {
        ...
        if (game->disc.rect.x >= x_max)
        {
            game->disc.step.x *= -1;
            game->p1.score += 1;
            g_snprintf(buffer, 10, "%i", game->p1.score);
            gtk_label_set_label(game->p1.label, buffer);
            set_pause(game);
        }


        else if (game->disc.rect.x == 0)
        {
            game->disc.step.x *= -1;
            game->p2.score += 1;
            g_snprintf(buffer, 10, "%i", game->p2.score);
            gtk_label_set_label(game->p2.label, buffer);
            set_pause(game);
        }
        ...
        if (game->p1.score == END_GAME_SCORE || game->p2.score == END_GAME_SCORE)
        {
            ...
            GtkWidget *dialog = gtk_message_dialog_new_with_markup(game->ui.window, flags,
                        GTK_MESSAGE_INFO, GTK_BUTTONS_CLOSE, 
                        "Player 1: %i point(s)\nPlayer 2: %i point(s)\n",
                        game->p1.score, game->p2.score);
            
            gtk_window_set_title(GTK_WINDOW(dialog), "Game Over");
            gchar *bold = g_markup_printf_escaped("<span weight=\"bold\"> The winner is player %i.</span>", 
                    (game->p1.score < game->p2.score ? 2 : 1));

            gtk_message_dialog_format_secondary_markup(GTK_MESSAGE_DIALOG(dialog), "%s", bold);
...
gboolean on_move_training(gpointer user_data) 
{
    Game* game = user_data;
    ...
    // make it so that it is in the middle and doesnt go out
...
void on_training(GtkCheckButton *button, gpointer user_data)
{
    Game* game = user_data;

    if (gtk_toggle_button_get_active((GtkToggleButton*) button))
        game->p1.event = g_timeout_add(game->disc.period, on_move_training, game);
    else
    {
...
gboolean on_configure(GtkWidget *widget, GdkEvent *event, gpointer user_data)
{
    ...
    game->p2.rect.y = CLAMP(game->p2.rect.y, 0, gtk_widget_get_allocated_height(widget) - game->p2.rect.height);
...
gboolean on_move_paddle_one(gpointer user_data)
{
    Game* game = user_data;
...
gboolean on_move_paddle_two(gpointer user_data)
{
    Game* game = user_data;
...
gboolean on_key_press(GtkWidget *widget, GdkEventKey *event, gpointer user_data)
{
    ...
    if (event->keyval == GDK_KEY_f && !gtk_toggle_button_get_active((GtkToggleButton*) game->ui.training_cb))
    {
        game->p1.step = -1*PADDLE_STEP;
        if (game->p1.event == 0)
            game->p1.event = g_timeout_add(PADDLE_PERIOD, on_move_paddle_one, game);
        return TRUE;
    }

    // If the 'v' key is pressed, moves the player 1 downwards.
    else if (event->keyval == GDK_KEY_v && !gtk_toggle_button_get_active((GtkToggleButton*) game->ui.training_cb))
    {
    ...
    {
        game->p2.step = -1*PADDLE_STEP;
        if (game->p2.event == 0)
            game->p2.event = g_timeout_add(PADDLE_PERIOD, on_move_paddle_two, game);
        return TRUE;
    }
...
gboolean on_key_release(GtkWidget *widget, GdkEventKey *event, gpointer user_data)
{
    ...
    if (event->keyval == GDK_KEY_v && !gtk_toggle_button_get_active((GtkToggleButton*) game->ui.training_cb))
    {
        if (game->p1.step > 0) {
            g_source_remove(game->p1.event);
            game->p1.event = 0;
...
int main (int argc, char *argv[])
{
    ...
    g_signal_connect(area, "configure_event", G_CALLBACK(on_configure), &game);
    
    g_signal_connect(training_cb, "clicked", G_CALLBACK(on_training), &game);
